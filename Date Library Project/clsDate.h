...
public:
	...
	clsDate() //Object 1 empty to date
	{
		time_t t = time(0);
		tm* now = localtime(&t);
	...
	clsDate(string sDate) //Object2 String To Date
	{
		vector <string> vDate;
	...
	clsDate(short Day,short Month,short Year ) //Parametrized by Day Month Year
	{
		_Day = Day;
		_Month = Month;
		_Year = Year;
	...
	clsDate(short DayOrderInYear,short Year) //Parametrized by Day Order In Year & Year
	{
	...
	{
		return (Date.Month >= 1 && Date.Month <= 12 && Date.Day >= 1 && Date.Day <= NumberOfDaysInAMonth(Date.Month,Date.Year));
	}
	...
	string DayShortName()
	{
		...
		return ArrDays[DayOfWeekOrder(_Day,_Month,_Year)];
	...
	static short DaysFromTheBeginningOfYear(short Day, short Month, short Year)
	{
		...
		{
			RestOfDays += NumberOfDaysInAMonth(Year, i);
		}
	...
	short DaysFromTheBeginningOfYear()
	{
		...
		{
			RestOfDays += NumberOfDaysInAMonth(_Year, i);
		}

		return  _Day + RestOfDays;
	...
	void AddDays(short Days)
	{
		...
		short RemainingDays = Days + DaysFromTheBeginningOfYear(_Day, _Month, _Year);
		short MonthDays = 0;
